...
import { useSocialAccountsQuery } from '@/hooks/queries/use-app-queries'
import {
...
const BUNDLE_ERROR_MESSAGES: Record<string, string> = {
  ...
  'not-enought-permissions': 'Insufficient permissions. Please try again and grant all requested access.',
  'insufficient-permissions': 'Insufficient permissions. Please try again and grant all requested access.',
}

function getBundleErrorMessage(code: string | null | undefined) {
  const normalized = normalizeBundleCallbackCode(code)
  if (!normalized) return null
  return (
    BUNDLE_ERROR_MESSAGES[normalized] ||
    (normalized.endsWith('permissions') ? BUNDLE_ERROR_MESSAGES['not-enough-permissions'] : null)
  )
}

function getChannelEntityLabel(platform: string) {
  switch (platform) {
    case 'YOUTUBE':
      return 'Channel'
    case 'FACEBOOK':
...
function mapApiSocialAccount(acc: any): SocialAccount {
  return {
    id: acc.id,
...
function canManageChannels(account: SocialAccount) {
  const channelCount = account.metadata?.available_channels?.length || 0
  return (
    platformRequiresChannelSelection(account.platform) &&
    (getIntegrationChannelSelectionState(account) || channelCount > 1)
  )
}

const PENDING_CONNECT_KEY = 'syncrio_pending_social_connect'

// Main Page 

function IntegrationsContent({ initialSocialAccounts }: { initialSocialAccounts?: any[] }) {
  const { user: session } = useAuth()
  const { collapsed, setUnreadNotificationsCount } = useAppShell()
  const accountsQuery = useSocialAccountsQuery(Boolean(session), initialSocialAccounts)
  const [accounts, setAccounts] = useState<SocialAccount[]>(() => 
    (initialSocialAccounts || []).map(mapApiSocialAccount)
  )
  ...
  const [pendingReturnPlatform, setPendingReturnPlatform] = useState<Platform | null>(null)

  const searchParams = useSearchParams()
  const router = useRouter()
  const loading = accountsQuery.isLoading && accounts.length === 0

  const transformedAccounts = useMemo<SocialAccount[]>(
    () => (accountsQuery.data || []).map(mapApiSocialAccount),
    [accountsQuery.data]
  )
  ...
  const fetchSocialAccounts = useCallback(async () => {
    ...
    try {
      ...
      const nextAccounts = (result.data || []).map(mapApiSocialAccount)
  ...
  const handleRefreshAccounts = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setMessage({ type: 'success', text: data.message || 'Accounts updated.' })
      } else {
        setMessage({ type: 'error', text: data.error || 'Could not refresh accounts right now.' })
      }
    } catch {
      setMessage({ type: 'error', text: 'Could not refresh accounts right now.' })
    } finally {
      setSyncLoading(false)
    }
  }, [session, fetchSocialAccounts])

  const handleFinalizeConnect = useCallback(async (platform: Platform) => {
    if (!session) return
    try {
      setSyncLoading(true)
      const res = await fetch('/api/social/accounts', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'finalize-connect', platform }),
      })
      const data = await res.json()

      if (!data.success) {
        throw new Error(data.error || 'We could not finish connecting this account yet.')
      }

      const latestAccounts = await fetchSocialAccounts()
      localStorage.removeItem(PENDING_CONNECT_KEY)
      setPendingReturnPlatform(null)

      const accountNeedingChannel = latestAccounts?.find(
        (account) => account.platform === platform && account.isConnected && getIntegrationChannelSelectionState(account)
      )

      if (accountNeedingChannel) {
        setSelectingChannelAccount(accountNeedingChannel)
        setMessage({
          type: 'warning',
          text: `Choose a ${getChannelEntityLabel(accountNeedingChannel.platform).toLowerCase()} to finish connecting ${PLATFORM_META[accountNeedingChannel.platform as Platform]?.displayName || accountNeedingChannel.platform}.`,
        })
      } else {
        setMessage({ type: 'success', text: `${PLATFORM_META[platform]?.displayName || platform} connected.` })
      }
    } catch {
      const refreshed = await fetchSocialAccounts()
      if (!refreshed) {
        setMessage({ type: 'error', text: 'We could not finish connecting this account yet. Please try again.' })
      }
      localStorage.removeItem(PENDING_CONNECT_KEY)
      setPendingReturnPlatform(null)
    } finally {
          ...
          setAccounts((prev) =>
            prev.map((account) =>
              account.id === accountId
                ? {
                    ...account,
                    accountId: data.data.account_id || account.accountId,
                    username: data.data.username || account.username,
                    displayName: data.data.display_name || account.displayName,
                    avatarUrl: data.data.avatar_url || account.avatarUrl,
                    metadata: data.data.metadata || account.metadata,
                  }
                : account
            )
          )
  ...
  const handleOpenChannelSelector = useCallback(async (account: SocialAccount) => {
    const hasCachedChannels = (account.metadata?.available_channels?.length || 0) > 0
    const shouldRefreshChannels =
      platformRequiresChannelSelection(account.platform) &&
      (!hasCachedChannels || canManageChannels(account))

    if (!shouldRefreshChannels) {
      setSelectingChannelAccount(account)
      return
    }
    ...
    try {
      ...
      const res = await fetch('/api/social/accounts', {
        ...
        body: JSON.stringify({
          ...
          socialAccountId: account.bundleSocialAccountId,
        }),
      })
      const data = await res.json()
      ...
      if (data.success && data.data) {
        ...
        setAccounts((prev) =>
          prev.map((existing) => (existing.id === account.id ? refreshedAccount : existing))
        )
        setSelectingChannelAccount(refreshedAccount)
      } else {
        setSelectingChannelAccount(account)
      }
    } catch {
  ...
  useEffect(() => {
    ...
    const connectedPlatform = searchParams.get('connectedPlatform') as Platform | null
    const errorParam = searchParams.get('error') || searchParams.get('message')
    const bundleErrorMessage = getBundleErrorMessage(errorParam)
    let pendingPlatform: Platform | null = null

    try {
      const rawPending = localStorage.getItem(PENDING_CONNECT_KEY)
      if (rawPending) {
        const parsed = JSON.parse(rawPending) as { platform?: Platform }
        pendingPlatform = parsed.platform || null
      }
    } catch {
      pendingPlatform = null
    }

    if (bundleErrorMessage) {
      localStorage.removeItem(PENDING_CONNECT_KEY)
      setPendingReturnPlatform(null)
      setMessage({ type: 'error', text: bundleErrorMessage })
      router.replace('/integrations')
      return
    }

    if (connectedPlatform || pendingPlatform) {
      const platform = connectedPlatform || pendingPlatform
      if (!platform) return
      setPendingReturnPlatform(platform)
      const t = setTimeout(() => handleFinalizeConnect(platform), 300)
      return () => clearTimeout(t)
    }
  }, [session, searchParams, handleFinalizeConnect, router])
    ...
    try {
      ...
      if (!response.ok || !data.success) {
        if (data.error === 'already_connected') {
          setMessage({ type: 'success', text: `This ${PLATFORM_META[platform]?.displayName} account is already connected.` })
      ...
      if (data.success && data.url) {
        localStorage.setItem(PENDING_CONNECT_KEY, JSON.stringify({
          platform,
          startedAt: Date.now(),
        }))
  ...
  useEffect(() => {
    setUnreadNotificationsCount(needsReauthCount)

    return () => {
      setUnreadNotificationsCount(3)
    }
  }, [needsReauthCount, setUnreadNotificationsCount])
      ...
      <div className={`max-w-[1400px] mx-auto px-3 sm:px-4 lg:px-6 pt-4 pb-8 ${collapsed ? 'ml-16' : 'ml-64'} transition-all duration-300`}>
        <main className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            ...
            <div className="flex items-center gap-3">
              <Button
                ...
                className="rounded-xl px-2.5 text-[0px] gap-0"
                aria-label={syncLoading ? 'Refreshing accounts' : 'Refresh accounts'}
                title={syncLoading ? 'Refreshing accounts' : 'Refresh accounts'}
              >
          ...
          {message && (
            <div className={`p-4 rounded-2xl border ${message.type === 'success'
              ...
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setMessage(null)} className="text-sm opacity-70 hover:opacity-100 ml-4">close</button>
              </div>
            </div>
          )}

          {pendingReturnPlatform && (
            <div className="rounded-2xl border border-indigo-200 dark:border-indigo-800 bg-indigo-50 dark:bg-indigo-900/20 p-4 flex items-center gap-3">
              ...
              <span className="text-sm text-indigo-800 dark:text-indigo-200 font-medium">
                Finalizing {PLATFORM_META[pendingReturnPlatform]?.displayName || pendingReturnPlatform} connection...
              </span>
            </div>
          )}
          ...
          {syncLoading && (
            <div className="rounded-2xl border border-indigo-200 dark:border-indigo-800 bg-indigo-50 dark:bg-indigo-900/20 p-4 flex items-center gap-3">
              <Loader2 className="h-5 w-5 text-indigo-600 animate-spin" />
              <span className="text-sm text-indigo-800 dark:text-indigo-200 font-medium">Updating connected accounts...</span>
            </div>
          )}
                    ...
                    <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {filtered.map(acc => (
                        <ConnectedAccountRow
                          ...
                          onSelectChannel={handleOpenChannelSelector}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {(Object.keys(PLATFORM_META) as Platform[]).map(p => (
                    <PlatformCard
                      ...
                      pendingConnection={pendingReturnPlatform === p}
...
function ConnectedAccountRow({
  ...
  const canChangeChannel = canManageChannels(account)
  const channelEntityLabel = getChannelEntityLabel(platform)
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-bold flex items-center gap-1.5 text-slate-900 dark:text-white">
            {account.displayName}
            {requiresChannel && (
              <Badge className="rounded-full border-amber-200 bg-amber-50 px-2 py-0 text-[9px] font-bold text-amber-700 hover:bg-amber-50 dark:border-amber-400/20 dark:bg-amber-500/10 dark:text-amber-300">
                Channel required
              </Badge>
            )}
            {account.needsReauth && (
          ...
          <div className="flex items-center gap-2">
            ...
            {canChangeChannel && (
              <Button
                variant="link"
                size="sm"
                className="h-auto p-0 text-[0px] text-indigo-600 dark:text-indigo-400 font-bold hover:underline"
                title={`${requiresChannel ? 'Select' : 'Change'} ${channelEntityLabel}`}
                aria-label={`${requiresChannel ? 'Select' : 'Change'} ${channelEntityLabel}`}
                onClick={() => onSelectChannel(account)}
              >
                <span className="text-[11px]">{requiresChannel ? `Select ${channelEntityLabel}` : `Change ${channelEntityLabel}`} →</span>
              </Button>
            )}
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            className="hover:text-red-600 h-8 w-8 text-neutral-400 transition-all hover:bg-red-50 dark:hover:bg-red-950/20 rounded-xl disabled:opacity-100"
          >
              ...
              <li key={acc.id} className="flex items-center gap-2 p-2 rounded-2xl border border-black/10 dark:border-white/10 bg-white/70 dark:bg-neutral-900/50">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-1.5">
                    ...
                    {getIntegrationChannelSelectionState(acc) && (
                      <Badge className="rounded-full border-amber-200 bg-amber-50 px-1.5 py-0 text-[9px] font-bold text-amber-700 hover:bg-amber-50 dark:border-amber-400/20 dark:bg-amber-500/10 dark:text-amber-300">
                        Needs channel
                      </Badge>
                    )}
                  </div>
                    {canManageChannels(acc) ? (
                      <button
                      ...
                      >
                        <Settings2 className="h-2.5 w-2.5" /> {`${getIntegrationChannelSelectionState(acc) ? 'Pick' : 'Change'} ${getChannelEntityLabel(acc.platform)}`}
                      </button>
                    ) : (
                ...
                <Button
                  ...
                  className="hover:text-red-600 h-7 w-7 opacity-50 hover:opacity-100 disabled:opacity-100"
                >
...
function ChannelSelectorDialog({
  ...
  const channelEntityLabel = getChannelEntityLabel(account.platform)
  const channelEntityLabelLower = channelEntityLabel.toLowerCase()
      ...
      <DialogContent className="sm:max-w-md rounded-3xl border-black/10 dark:border-white/10 bg-white/90 dark:bg-neutral-950/90 backdrop-blur-3xl shadow-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Select {channelEntityLabel}
          </DialogTitle>
          <DialogDescription className="text-sm">
            Choose which {meta?.displayName} {channelEntityLabelLower} you want to connect to Syncrio.
          </DialogDescription>
        </DialogHeader>
...
export default function IntegrationsPageContent({ initialSocialAccounts }: { initialSocialAccounts?: any[] }) {
  return (
    <Suspense fallback={<div className="h-screen flex items-center justify-center"><Loader2 className="h-8 w-8 animate-spin opacity-20" /></div>}>
      <IntegrationsContent initialSocialAccounts={initialSocialAccounts} />
    </Suspense>
  )
}
