...
import { appQueryKeys, usePostAnalyticsQuery, usePostsQuery } from '@/hooks/queries/use-app-queries'
...
import { showGlobalToast } from '@/hooks/use-toast'

const AdvancedEditModal = dynamic(
  () => import('@/components/content/advanced-edit-modal').then((mod) => mod.AdvancedEditModal),
  { ssr: false }
)
const ScheduleModal = dynamic(
  () => import('@/components/content/schedule-modal').then((mod) => mod.ScheduleModal),
  { ssr: false }
)
const CreatePostModal = dynamic(
  () => import('@/components/create-post/modal/create-post-modal').then((mod) => mod.CreatePostModal),
  { ssr: false }
)
...
export default function PostsPage({ initialData }: { initialData?: any }) {
  const { user: session } = useAuth()
  const { collapsed } = useAppShell()
  const queryClient = useQueryClient()
  const postsQuery = usePostsQuery(Boolean(session), initialData)
  const postIds = useMemo(
    () => ((postsQuery.data?.posts as Post[] | undefined) || []).map((post) => post.id),
    [postsQuery.data?.posts]
  )
  const postAnalyticsQuery = usePostAnalyticsQuery(postIds, Boolean(session) && postIds.length > 0)
  ...
  const invalidateSharedPostQueries = async () => {
    await Promise.all([
  ...
  useEffect(() => {
    if (postsQuery.data?.posts) {
      setPosts(postsQuery.data.posts as unknown as Post[])
      setLoading(false)
  ...
  useEffect(() => {
    ...
    setPosts((currentPosts) =>
      currentPosts.map((post) => ({
        ...post,
        analytics: (postAnalyticsQuery.data?.[post.id] as any) || [],
      }))
    )
  }, [postAnalyticsQuery.data])
    ...
    <div className="min-h-screen font-sans bg-gradient-to-b from-neutral-50 to-white dark:from-neutral-950 dark:to-neutral-900 text-slate-900 dark:text-slate-100 transition-all duration-200">
      ...
      <div className="flex items-center justify-between mb-1">
        ...
        <button
          ...
          className="px-3 py-2 rounded-full bg-gradient-to-r from-rose-500 to-pink-500 hover:from-rose-600 hover:to-pink-600 text-white flex items-center gap-2 shadow text-sm hover:scale-105 active:scale-95 transition-all duration-200"
        >
        ...
        <div className="rounded-3xl p-16 bg-white/60 dark:bg-neutral-900/60 border border-black/5 dark:border-white/5 shadow-lg text-center hover:shadow-xl transition-all duration-200">
          ...
          <p className="text-sm opacity-60 mb-8 max-w-md mx-auto hover:opacity-80 transition-opacity duration-200">
                ...
                <button
                  type="button"
                  onClick={() => setShowCreatePostModal(true)}
                  className="px-4 py-2 rounded-full bg-gradient-to-r from-rose-500 to-pink-500 hover:from-rose-600 hover:to-pink-600 text-white flex items-center gap-2 mx-auto hover:scale-105 active:scale-95 transition-all duration-200"
                >
