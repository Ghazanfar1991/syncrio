...
export async function postTweetWithVideo(
  ...
  try {
    ...
    if (tweetData?.media?.media_ids && Array.isArray(tweetData.media.media_ids) && tweetData.media.media_ids.length >= 1) {
      try {
        console.log('[Twitter] Using v1.1 statuses/update for media tweet...')
        if (userId && accountId) {
          ...
          console.log('[Twitter] v1.1 tweet posted successfully (DB creds):', result)
          return { id: result.id, text: uniqueContent }
        } else {
          const { postTweetWithMediaIdsV11Env } = await import('./twitter-multi')
          const result = await postTweetWithMediaIdsV11Env({ text: uniqueContent, mediaIds: tweetData.media.media_ids })
          console.log('[Twitter] v1.1 tweet posted successfully (env creds):', result)
          return { id: result.id, text: uniqueContent }
        }
      } catch (e) {
        console.warn('[Twitter] v1.1 media posting failed; falling back to v2...', e)
      }
    }
...
export async function postTweet(
  ...
  if (tweetData?.media?.media_ids && Array.isArray(tweetData.media.media_ids) && tweetData.media.media_ids.length >= 1) {
    try {
      console.log('🐦 [DEBUG] Using v1.1 statuses/update for media tweet...')
      if (userId && accountId) {
        const { postTweetWithMediaIdsV11 } = await import('./twitter-multi')
        const result = await postTweetWithMediaIdsV11({ text: uniqueContent, mediaIds: tweetData.media.media_ids, userId, accountId })
        console.log('🐦 [DEBUG] v1.1 tweet posted successfully (DB creds):', result)
        return { id: result.id, text: uniqueContent }
      } else {
        const { postTweetWithMediaIdsV11Env } = await import('./twitter-multi')
        const result = await postTweetWithMediaIdsV11Env({ text: uniqueContent, mediaIds: tweetData.media.media_ids })
        console.log('🐦 [DEBUG] v1.1 tweet posted successfully (env creds):', result)
        return { id: result.id, text: uniqueContent }
      }
    } catch (e) {
      console.warn('🐦 [DEBUG] v1.1 media posting failed; falling back to v2...', e)
    }
  }
