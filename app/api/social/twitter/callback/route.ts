  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(
        `${process.env.NEXTAUTH_URL}/dashboard?error=twitter_oauth_failed`
      )
    }
    ...
    try {
      ...
      return NextResponse.redirect(
        `${process.env.NEXTAUTH_URL}/integrations?success=twitter_connected`
      )
    } catch (error) {
      console.error('Twitter token exchange failed:', error)
      return NextResponse.redirect(
        `${process.env.NEXTAUTH_URL}/integrations?error=twitter_connection_failed`
      )
    }
  } catch (error) {
